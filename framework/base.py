...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = Distill(x_init, y_init, args.arch, args.window, args.totwindow, args.d, args.inner_lr, args.num_train_eval, img_pc=args.num_per_class, 
    ...
    for epoch in range(args.start_epoch, args.epochs):
        start_curriculum , window_size= model.module.window_start_size(stage)
        # initialize the EMA
        if epoch == 0: model.module.ema_init(args.clip_coef)  
        grad_tmp, losses_avg, distill_steps, acc_average= train(train_loader, model, criterion, optimizer, epoch, device, distill_steps, args, start_curriculum, window_size, stage)
        grad_acc.append(grad_tmp)
        print('The current update step is {}'.format(distill_steps))
        if args.rank == 0:
            print("epoch=",epoch, ",accuracy=", acc_average)        
        if stage == 1:
            if acc_average - old_acc_average < 1.5:
                flag_stage2 += 1
            else:
                flag_stage2 = 0
            if flag_stage2 == 0.15 * args.totwindow:
                stage = 2
        if stage == 2:
            if acc_average - old_acc_average < 1:
                flag_stage3 += 1
            else:
                flag_stage3 = 0
            if flag_stage3 == 0.125 * args.totwindow:
                stage = 3
        old_acc_average = acc_average
...
def train(train_loader, model, criterion, optimizer, epoch, device, distill_steps, args, start_curriculum, window_size,stage):
    acc_sum = 0
    acc_count = 0
    ...
    if model.module.cctype == 2:
        if args.rank == 0:
            shared_curriculum = torch.tensor(start_curriculum)
            shared_window = torch.tensor(window_size)
        else:
            shared_curriculum = torch.tensor(0)
            shared_window = torch.tensor(args.window)
        shared_curriculum = shared_curriculum.to(device)
        shared_window = shared_window.to(device)
        if args.mp_distributed:
            dist.broadcast(shared_curriculum, src=0)
            dist.broadcast(shared_window, src=0)
        
        model.module.curriculum = shared_curriculum.item()
        model.module.window = shared_window.item()
        
    print('GPU_{}_using curriculum {} with window {} with stage {}'.format(args.rank, model.module.curriculum, model.module.window, stage))
    ...
    return grad_acc, losses.avg, distill_steps, round(acc_sum / acc_count, 2)
